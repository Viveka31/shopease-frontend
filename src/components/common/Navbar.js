...
import {
  ...
  FiChevronDown, FiUser, FiPlusCircle, FiShoppingBag
} from 'react-icons/fi';
...
function SellerNavbar() {
  ...
  const isActive = (path) => location.pathname.startsWith(path) ? 'seller-nav-link active' : 'seller-nav-link';
  ...
  return (
    <header className="seller-navbar">
      <div className="container seller-navbar-inner">
        ...
        {/* Desktop nav links */}
        <nav className="seller-nav-links">
          ...
          <Link to="/seller/store-profile" className={isActive('/seller/store-profile')}>
            <FiSettings /> Store Profile
          </Link>
        </nav>
              ...
              <div className="dropdown-menu">
                <div className="dropdown-header">
                  ...
                  <div>
                    ...
                    <span className="badge badge-teal">seller</span>
                  </div>
                </div>
                <div className="dropdown-divider" />
                <Link to="/seller/dashboard"     className="dropdown-item" onClick={() => setDropdownOpen(false)}><FiBarChart2 />  Dashboard</Link>
                <Link to="/seller/products"      className="dropdown-item" onClick={() => setDropdownOpen(false)}><FiPackage />    Products</Link>
                <Link to="/seller/orders"        className="dropdown-item" onClick={() => setDropdownOpen(false)}><FiShoppingBag />Orders</Link>
                <Link to="/seller/store-profile" className="dropdown-item" onClick={() => setDropdownOpen(false)}><FiSettings />   Store Profile</Link>
                <div className="dropdown-divider" />
      ...
      {mobileOpen && (
        <div className="mobile-menu">
          <Link to="/seller/dashboard"     className="mobile-seller-link"><FiBarChart2 />  Dashboard</Link>
          <Link to="/seller/products"      className="mobile-seller-link"><FiPackage />    My Products</Link>
          <Link to="/seller/products/add"  className="mobile-seller-link"><FiPlusCircle /> Add Product</Link>
          <Link to="/seller/orders"        className="mobile-seller-link"><FiShoppingBag />Orders</Link>
          <Link to="/seller/store-profile" className="mobile-seller-link"><FiSettings />   Store Profile</Link>
          <button className="mobile-seller-link danger-link" onClick={handleLogout}><FiLogOut /> Logout</button>
        </div>
      )}
    </header>
  );
                ...
                <div className="user-dropdown" ref={dropdownRef}>
                  ...
                  {dropdownOpen && (
                    <div className="dropdown-menu">
                      ...
                      <div className="dropdown-divider" />
                      <Link to="/profile" className="dropdown-item" onClick={() => setDropdownOpen(false)}><FiUser />    Profile</Link>
...
// ── Export: auto-switch based on role ─────────────────────────────────────────
export default function Navbar() {
  const { user } = useAuth();
  const isSeller = user?.role === 'seller' || user?.role === 'admin';
  return isSeller ? <SellerNavbar /> : <BuyerNavbar />;
}
